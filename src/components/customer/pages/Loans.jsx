...
import React, { useState } from "react";
import { applyLoan, getLoansByCustomerId } from "../../customer/services/loanService";
import "./Loans.css";
...
export default function Loans() {
  ...
  const [success, setSuccess] = useState(null);
  const [isLoading, setIsLoading] = useState(false);
  const [customerLoans, setCustomerLoans] = useState([]); // ✅ fetched loans
  ...
  const validate = () => {
    ...
    if (!formData.LoanTypeId) tempErrors.LoanTypeId = "Please select loan type";
    if (!formData.AppliedAmount) tempErrors.AppliedAmount = "Amount is required";
    else if (parseFloat(formData.AppliedAmount) < 10000)
      tempErrors.AppliedAmount = "Minimum loan amount ₹10,000";
    if (!formData.TenureMonths) tempErrors.TenureMonths = "Tenure is required";
    if (!formData.Purpose) tempErrors.Purpose = "Purpose is required";
  ...
  const fetchLoans = async () => {
    if (!formData.CustomerId) {
      setErrors({ CustomerId: "Enter Customer ID to fetch loans" });
      return;
    }
    try {
      const res = await getLoansByCustomerId(formData.CustomerId);
      setCustomerLoans(res.data);
    } catch (err) {
      console.error(err);
      setErrors({ general: "Failed to fetch loans for this customer." });
    }
  };
  ...
  return (
    <div className="customer-loans">
      ...
      {/* <div className="customer-loans-list">
        ...
        <button className="btn fetch" onClick={fetchLoans}>
          🔍 Fetch Loans for Customer
        </button>
        {customerLoans.length > 0 && (
          <ul className="loan-list">
            {customerLoans.map((loan) => (
              <li key={loan.LoanAppId}>
                <b>Loan #{loan.LoanAppId}</b> - ₹{loan.AppliedAmount} |{" "}
                {loanTypes.find((t) => t.id === loan.LoanTypeId)?.name} |{" "}
                {loan.TenureMonths} months | Status: {loan.Status}
              </li>
            ))}
          </ul>
        )}
